...
const Profits = () => {
  ...
  const recordSquarePayment = async (row = null) => {
    const paymentId = window.prompt('Enter the Square payment ID from the transaction details:');
    if (!paymentId?.trim()) return;
    if (!row && !window.confirm('Add this payment as a new profit record? If an income row already exists for it, cancel and use its Sync Square button to avoid double counting.')) return;
    setSaving(true);
    try {
      const response = await fetch(`${apiUrl}/api/profits/square-payment`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('portalToken') || ''}` },
        body: JSON.stringify({ paymentId: paymentId.trim(), profitId: row?.id || null }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data?.error || 'Could not sync this Square payment.');
      await fetchTransactions();
      setMessage({ type: 'success', text: `Square payment recorded: ${money.format(Number(data.gross_amount))} charged, ${money.format(Number(data.fee_amount))} fee, ${money.format(Number(data.net_amount))} net.` });
    } catch (error) { setMessage({ type: 'error', text: error.message || 'Could not sync this Square payment.' }); }
    finally { setSaving(false); }
  };
