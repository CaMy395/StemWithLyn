...
const minutesFromTime = (time) => { const [hours, minutes] = timeKey(time).split(":").map(Number); return (hours * 60) + (minutes || 0); };
...
const ClientSchedulingPage = ({ portalMode = false }) => {
  ...
  const clientAuthHeaders = useMemo(() => { const id = loggedInUser?.id || localStorage.getItem("userId"); const username = loggedInUser?.username || localStorage.getItem("username"); const token = localStorage.getItem("portalToken"); return { ...(id ? { "x-user-id": String(id) } : {}), ...(username ? { "x-username": String(username) } : {}), ...(token ? { Authorization: `Bearer ${token}` } : {}) }; }, [loggedInUser]);
  const today = useMemo(() => { const date = new Date(); date.setHours(0, 0, 0, 0); return date; }, []);
  const firstBookableDate = useMemo(() => { const date = new Date(today); date.setDate(date.getDate() + 1); return date; }, [today]);
  ...
  const packageBalance = packages.find((item) => item.title === selectedAppointmentType);

  useEffect(() => {
    if (!isPortal) return;
    axios.get(`${apiUrl}/client/packages`, { headers: clientAuthHeaders })
      .then(({ data }) => setPackages(Array.isArray(data) ? data : []))
      .catch(() => setPackages([]));
  }, [apiUrl, clientAuthHeaders, isPortal]);
  ...
  useEffect(() => {
    if (!isPortal) return;
    const requestedType = searchParams.get('appointmentType');
    if (packages.some((item) => item.title === requestedType && item.remaining > 0)) setSelectedAppointmentType(requestedType);
  ...
  const fetchAvailability = useCallback(async () => {
    ...
    try {
      ...
      setAvailableSlots((Array.isArray(availabilityRes.data) ? availabilityRes.data : [])
        .map((slot) => ({ ...slot, start_time: normalizeTime(slot.start_time), end_time: normalizeTime(slot.end_time) }))
        .filter((slot) => {
          const slotStart = minutesFromTime(slot.start_time);
          const slotEnd = minutesFromTime(slot.end_time);
          const hitsBlock = blockIntervals.some((block) => overlaps(slotStart, slotEnd, minutesFromTime(block.start), minutesFromTime(block.end)));
          const hitsAppointment = appointments.some((appt) => {
            const appointmentStart = minutesFromTime(appt.time);
            const appointmentEnd = appt.end_time ? minutesFromTime(appt.end_time) : appointmentStart + 30;
  ...
  const handleSubmit = async (event) => {
    ...
    if (toDateKey(selectedDate) <= toDateKey(today)) return setFormErr("Same-day booking is unavailable. Please choose tomorrow or a later date.");
    ...
    <form className="scheduler-shell" onSubmit={handleSubmit} noValidate>
      <div className="scheduler-main">
        <section className="booking-section"><div className="section-heading"><span className="step-number">1</span><div><h2>Choose a service</h2><p>Select the session that best fits your goals.</p></div></div>
          {categories.map((category) => <div className="service-category" key={category}><h3>{category}</h3><div className="service-grid">{appointmentTypes.filter((item) => item.category === category && (!item.title.includes("SCHEDULING") || (isPortal && packages.some((balance) => balance.title === item.title && balance.remaining > 0)))).map((item) => { const meta = getServiceMeta(item); const selected = item.title === selectedAppointmentType; return <button className={`service-card${selected ? " selected" : ""}`} type="button" key={item.title} onClick={() => { setSelectedAppointmentType(item.title); setSelectedSlot(null); setFormErr(""); }} aria-pressed={selected}><span className="service-check">{selected ? "✓" : ""}</span><strong>{item.title}</strong><span className="service-meta"><span>{meta.duration}</span><span>{packages.find((entry) => entry.title === item.title)?.remaining != null ? `${packages.find((entry) => entry.title === item.title).remaining} remaining` : meta.price}</span></span></button>; })}</div></div>)}
        </section>
